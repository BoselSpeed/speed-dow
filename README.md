...
- **تنبيه التحميل** (`src/components/DownloadGuide.tsx`): بطاقة تحذير من أزرار التنزيل داخل الإعلانات، معروضة **فوق منطقة التنزيل** حتى يراها الزائر قبل تنزيل الملف.
- **خطوات التثبيت** (`src/components/InstallGuideModal.tsx`): لوحة (Modal) تعرض خطوات تعطيل Play Protect وتنصيب الـ APK تلقائيًا عند دخول زائر على هاتف/تابلت (جهاز بلمس). يمكن للزائر إغلاقها بزر «فهمت، أُكمل»، ويُحفظ قرار الإغلاق في LocalStorage، ويتاح دائمًا زر «إعادة عرض خطوات التثبيت» أسفل بطاقة التحميل.
- كل النصوص هذه في `src/i18n/translations.ts`.

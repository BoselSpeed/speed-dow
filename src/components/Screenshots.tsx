      ...
      <div className="p-2.5">
        ...
        {shot.kind === 'mixed' && (
          <div className="space-y-1.5">
            {shot.covers?.map((c, i) => (
              <div key={i} className="flex items-center gap-1.5 rounded-md p-1.5" style={{ backgroundColor: 'var(--color-surface-2)' }}>
                <img src={c} alt="" className="h-10 w-8 rounded object-cover" loading="lazy" />
                <span className="h-1.5 flex-1 rounded-full" style={{ backgroundColor: 'var(--color-surface-2)' }} />
              </div>
            ))}
          </div>
        )}
...
export default function Screenshots() {
  ...
  return (
    <section id="screenshots" className="mx-auto max-w-6xl scroll-mt-24 px-4 py-20 sm:px-6">
      <Reveal>
        <div className="mx-auto max-w-2xl text-center">
          <span className="text-sm font-bold uppercase tracking-widest" style={{ color: 'var(--color-primary)' }}>{t.screenshotsEyebrow}</span>
          <h2 className="mt-2 text-3xl font-extrabold sm:text-4xl" style={{ color: 'var(--color-text)' }}>{t.screenshotsTitle}</h2>
      ...
      <div className="mt-12 grid gap-8 sm:grid-cols-3">
        {shots.map((s, i) => (
          <Reveal key={s.key} delay={i * 130}>
            ...
            >
              <Frame shot={s} />
              <h3 className="mt-4 text-lg font-bold transition-colors group-hover" style={{ color: 'var(--color-text)' }}>{s.title}</h3>
              <p className="mt-1 text-sm" style={{ color: 'var(--color-text-secondary)' }}>{s.desc}</p>
      ...
      {lightbox && (
        ...
        >
          <div className="lightbox-in relative max-w-sm" onClick={(e) => e.stopPropagation()}>
            <Frame shot={lightbox} />
            <div className="mt-4 rounded-xl p-4 text-center" style={{ backgroundColor: 'var(--color-bg)' }}>
              <h3 className="text-lg font-bold" style={{ color: 'var(--color-text)' }}>{lightbox.title}</h3>
              <p className="mt-1 text-sm" style={{ color: 'var(--color-text-secondary)' }}>{lightbox.desc}</p>
            </div>
            <button
              onClick={() => setLightbox(null)}
